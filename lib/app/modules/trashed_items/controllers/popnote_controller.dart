...
class PopnoteController extends GetxController {
  String? module_id;
  String? note_id;
  PopnoteController({this.module_id, this.note_id});

    
 Future<void> sendRequest(String action) async {
   try{
      if (action == 'restore') {
      final String url = 'http://192.168.8.101:8000/note/restore/$note_id';
      ...
      final String url = 'http://192.168.8.101:8000/note/delete';
      final response = await http.post(
        Uri.parse(url),
        body: {'module_id': module_id,'note_id': note_id},
      );
  ...
  void handleMenuSelection(BuildContext context, String choice) {
    if (choice == 'Restore') {
      _showAlertDialog(context, 'Restore', 'Do you want to restore this note?');
    } else if (choice == 'Delete') {
      _showAlertDialog(context, 'Delete', 'Do you want to delete this note permanently?');
    }
  }
